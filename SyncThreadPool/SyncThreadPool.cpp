#include "SyncThreadPool.h"

SyncThreadPool::SyncThreadPool(int min, int max, int queMaxSize)
    : m_min(min),
      m_max(max),
      ...
      m_exitThreadNum(0),
      m_stop(false) {
  m_manager = std::make_unique<std::thread>(&SyncThreadPool::manager, this);
  for (int i = 0; i < m_curThreadNum; ++i) {
    auto th = std::thread(&SyncThreadPool::worker, this);
    std::thread::id id = th.get_id();
    m_threadPoolMap[id] = std::move(th);
  }
}

void SyncThreadPool::worker() {
  while (!m_stop.load()) {
    taskCallBack task;
